...
def train(train_data, ngram_range=(1,1)):
    ...
    clf = SGDClassifier(loss="hinge", alpha=1e-3, n_iter=5, random_state=42).fit(X_train_counts, train_data.target)
...
def train_with_tf_idf(train_data, ngram_range=(1,1)):
    ...
    clf = SGDClassifier(loss="hinge", alpha=1e-3, n_iter=5, random_state=42).fit(X_train_tfidf, train_data.target)
    return clf,count_vect
